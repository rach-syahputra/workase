...
export function JobShareComponent({ job }: { job: JobDetail }) {
  ...
  const hanldeShareDirect = (platform: 'linkedin' | 'facebook') => {
    switch (platform) {
      case 'linkedin':
        window.open(
          `https://www.linkedin.com/feed/?linkOrigin=LI_BADGE&shareActive=true&shareUrl=${encodeURIComponent(jobUrl)}`,
          '_blank',
        );
        break;
      case 'facebook':
  ...
  const handleShare = async () => {
    ...
    switch (selectedPlatform) {
      case 'twitter':
        window.open(
          `https://twitter.com/intent/tweet?text=${encodeURIComponent(messageWithUrl)}`,
          '_blank',
        );
        break;
      case 'whatsapp':
        window.open(
          `https://wa.me/?text=${encodeURIComponent(messageWithUrl)}`,
          '_blank',
        );
        break;
      default:
